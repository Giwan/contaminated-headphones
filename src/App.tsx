...
export default function App() {
  ...
  const focusMainHeading = () => {
    setTimeout(() => mainHeadingRef?.focus(), 50);
  };
        ...
        <Show
          ...
          fallback={
            ...
            >
              {(data) => (
                <>
                  <h2 ref={mainHeadingRef} tabindex="-1" class="sr-only focus:not-sr-only">{data().manufacturer} {data().model}</h2>
                  <HeadphoneDetails
                    headphone={data()}
                    onReset={() => { setSelected(null); focusMainHeading(); }}
